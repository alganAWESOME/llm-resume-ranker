...
class OldResumeSorter:
    ...
    def generate_random_pairs(self, num_indices, n):
        """Generates random pairs of indices. Resumes at these indices will be compared."""
        if n > (num_indices * (num_indices - 1)) // 2:
            raise ValueError("Number of pairs requested exceeds the possible distinct pairs.")
        
        pairs = set()
        
        while len(pairs) < n:
            a, b = random.sample(range(num_indices), 2)
            if a > b:
                a, b = b, a
            pairs.add((a, b))
...
if __name__ == '__main__':
    ...
    # (opponent, outcome), outcome: 1=win, 0=loss, 0.5=draw
    matches_player1 = [(player2, 1), (player3, 0)]
    matches_player2 = [(player1, 0), (player3, 1)]
    matches_player3 = [(player1, 1), (player2, 0)]

    # Update ratings based on matches
    player1.update_player(matches_player1)
    player2.update_player(matches_player2)
    player3.update_player(matches_player3)
